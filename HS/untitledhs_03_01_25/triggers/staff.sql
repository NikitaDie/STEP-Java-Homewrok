...
--delete
CREATE OR REPLACE FUNCTION record_deleted_entity()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO deleted_entities (entity_name, entity_id, deleted_by, deleted_at, reason)
    VALUES ('staff', OLD.id, CURRENT_USER, NEW.deleted_at,'Soft delete');
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER staff_soft_delete_trigger
    AFTER UPDATE ON staff
    FOR EACH ROW
    WHEN (OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL)
    EXECUTE FUNCTION record_deleted_entity();

--update
CREATE OR REPLACE FUNCTION record_updated_entity()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO updated_entities (entity_name, entity_id, updated_by, updated_at, changes)
    VALUES ('staff', OLD.id, CURRENT_USER, NOW(),
            jsonb_strip_nulls(
                    jsonb_build_object(
                            'firstname',
                            ...
                            CASE WHEN OLD.surname IS DISTINCT FROM NEW.surname
                                ...
                                ELSE NULL END,
                            'job_position_id',
                            CASE WHEN OLD.job_position_id IS DISTINCT FROM NEW.job_position_id
                                THEN jsonb_build_object('old', OLD.job_position_id, 'new', NEW.job_position_id)
                                ELSE NULL END,
                            'salary',
                            CASE WHEN OLD.salary IS DISTINCT FROM NEW.salary
                                THEN jsonb_build_object('old', OLD.salary, 'new', NEW.salary)
                                ELSE NULL END,
                            'phone',
...
CREATE TRIGGER staff_update_trigger
    AFTER UPDATE ON staff
    FOR EACH ROW
EXECUTE FUNCTION record_updated_entity();
